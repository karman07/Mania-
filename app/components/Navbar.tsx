...
function UserMenu({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      ref={ref}
      className="absolute right-0 top-[calc(100%+10px)] w-52 rounded-2xl border border-saffron/15 dark:border-cream/10 bg-cream dark:bg-[#110B20] shadow-xl shadow-ink/10 dark:shadow-ink/40 overflow-hidden z-[60]"
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || !isLanding
          ? "bg-cream/95 dark:bg-[#0C0818]/95 backdrop-blur-md shadow-sm border-b border-saffron/10 dark:border-cream/5"
          : "bg-transparent"
      }`}
    >
          ...
          {!searchOpen && (
            <div className="hidden md:flex items-center gap-7">
              ...
              <div ref={genreRef} className="relative">
                ...
                {genreOpen && (
                  <div
                    className="absolute top-[calc(100%+12px)] left-1/2 -translate-x-1/2 w-64 rounded-2xl border border-saffron/15 dark:border-cream/10 bg-cream dark:bg-[#110B20] shadow-xl shadow-ink/10 dark:shadow-ink/40 overflow-hidden z-[60] p-2 grid grid-cols-2 gap-0.5"
                    style={{ animation: "dropIn 0.16s cubic-bezier(0.34,1.4,0.64,1)" }}
                  >
